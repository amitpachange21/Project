...
# Train or test 
mode = 'train'
directory = 'data/'+mode+'/'# here we are saving into specific directory

cap = cv2.VideoCapture(0)#0 means our primary webcam of laptop 
...
while True:
    ...
    _, roi = cv2.threshold(roi, 120, 255, cv2.THRESH_BINARY)# pure black and white
    cv2.imshow("ROI", roi)
    
    interrupt = cv2.waitKey(10)#write image into specific directory based on key pressing
